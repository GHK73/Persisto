...
export const resetPassword = (email, newPassword) => api.post('/forgot-password/reset', { email, newPassword });

// Updated: use handle (username) instead of userId for profile pic upload and stats fetch
export const updateProfilePicture = (handle, formData) => api.post(`/users/${handle}/profile-pic`, formData, {
  headers: {
    'Content-Type': 'multipart/form-data',
  },
}).then(res => res.data);

export const getUserStats = (handle) => api.get(`/users/${handle}/stats`).then(res => res.data);
